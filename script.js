...
if(userData){
    recentlyViewed = JSON.parse(userData)
}
}
function loadCartData(){
  const cartData=localStorage.getItem("cartData")
if(cartData){
    recentlyViewed = JSON.parse(cartData)
    updateCartCount()
}
}
...
if(ordersData){
    recentlyViewed = JSON.parse(ordersData)
}
}

// Save user data (small helper expected elsewhere in the codebase)
